...
#
# sub()      return a copy of the expresion with x substituted for c
# Example:
...
class Or():
    ...
    def sub(self, x, v):
        return Or(self.lhs.sub(x, v), self.rhs.sub(x, v))
...
class Arrow():
    ...
    def sub(self, x, v):
        return Arrow(self.lhs(x, v), self.rhs(x, v)) 
...
class Pred():
    def __init__(self, n, vs):
        self.name = n
        self.vars = vs #List of arguments for the predicate
    ...
    def sub(self, x, v):
        def f(n): 
            if n == x:
                ...
                return n
        return Pred(self.name , [f(var) for var in self.vars]) #for each variable in vs, if it equals the thing we're trying to replace, return the replacing value, v. Otherwise we keep n
